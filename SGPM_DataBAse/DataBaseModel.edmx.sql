...
-- --------------------------------------------------
-- Date Created: 04/04/2024 10:34:02
-- Generated from EDMX file: C:\Users\jesus\GitHub\SGPM_Server\SGPM_DataBAse\DataBaseModel.edmx
-- --------------------------------------------------
...
GO
IF OBJECT_ID(N'[dbo].[FK_Solicitud_Beneficiario]', 'F') IS NOT NULL
    ALTER TABLE [dbo].[SolicitudSet] DROP CONSTRAINT [FK_Solicitud_Beneficiario];
GO
...
-- Creating foreign key on [BeneficiarioId] in table 'SolicitudSet'
ALTER TABLE [dbo].[SolicitudSet]
ADD CONSTRAINT [FK_Solicitud_Beneficiario]
...
-- Creating non-clustered index for FOREIGN KEY 'FK_Solicitud_Beneficiario'
CREATE INDEX [IX_FK_Solicitud_Beneficiario]
ON [dbo].[SolicitudSet]
    ([BeneficiarioId]);
GO
